# https://gist.github.com/1550309
# https://github.com/andi/simple-navigation/wiki/Twitter-Bootstrap-integration
#
...
class BootstrapTopbarList < SimpleNavigation::Renderer::Base
  ...
  def tag_for(item)
    if item.url.nil?
      ...
      name = item.name
      name += '<b class="caret"></b>' if include_sub_navigation?(item) && !options[:is_subnavigation]
  ...
  def link_options_for(item)
    ...
    opts['data-toggle'] = 'dropdown' if include_sub_navigation?(item) && !options[:is_subnavigation]
    opts
  end
